...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.PreviewCallback {
    ...
    private CameraRecorder mMp4Recorder;
...
//                if (mVideoEncoder != null) {
//                    mVideoEncoder.putYUVData(rotateYUV420Degree90(data, mBestSize.width,
//                            mBestSize.height));
//                    //Toast.makeText(mContext, "开始录制视频成功", Toast.LENGTH_SHORT).show();
//                }
                if (mMp4Recorder != null) {
                    mMp4Recorder.feedData(rotateYUV420Degree90(data, mBestSize.width,
                            mBestSize.height));
                }
                break;
        }
...
//        try {
//            mMuxer = new MediaMuxerWrapper(getOutputMediaFile(MEDIA_TYPE_VIDEO));
//            if (true) {
//                // for video capturing
//                new MediaVideoEncoder(mMuxer, mMediaEncoderListener, mBestSize.width, mBestSize.height);
//            }
//            if (true) {
//                // for audio capturing
...
//    }
    private void startRecording() {
        if (mMp4Recorder == null) {
            mMp4Recorder = new CameraRecorder();
        }

        String savePath = getOutputMediaFile(MEDIA_TYPE_VIDEO);
        mMp4Recorder.setSavePath(savePath);
        try {
            mMp4Recorder.prepare(mBestSize.width, mBestSize.height);
            mMp4Recorder.start();

        } catch (Exception e) {
            Log.e(TAG, "startRecording: " + e.toString());
        }
    }
...
//    }
    ...
    public String getOutputMediaFile(int mediaType) {
