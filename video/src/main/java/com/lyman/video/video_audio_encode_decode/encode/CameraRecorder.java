...
//        mHeadInfo=null;
        ...
        videoFormat.setInteger(MediaFormat.KEY_COLOR_FORMAT, MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar);    // API >= 18
...
//                addADTStoPacket(temp,temp.length);
    ...
    private boolean videoStep(byte[] data) throws IOException {
        ...
        if (index >= 0) {
            ...
            buffer.clear();
            buffer.put(yuv);
            mVideoEnc.queueInputBuffer(index, 0, yuv.length, (System.nanoTime() - nanoTime) / 1000,
